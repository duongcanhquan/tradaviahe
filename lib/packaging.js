...
export function defaultIngredientPackHint(baseUnit) {
  ...
  if (u === "bao" || u === "bao thuốc") {
    return { packLabel: "cây thuốc", packFactor: "10" };
  }
  return { packLabel: "thùng", packFactor: "24" };
}

/**
 * Preset setup bán lẻ + kiện (thuốc bao/cây, nước chai/thùng…).
 * canSellPack: thành phẩm bán cả kiện trên POS; NL thường chỉ nhập kiện.
 */
export function buildRetailPackPreset({
  baseUnit = "bao",
  packLabel,
  packFactor,
  retailPrice = 0,
  ...
  const hint = defaultIngredientPackHint(baseUnit);
  const base = String(baseUnit || "bao").trim() || "bao";
  const pack = String(packLabel || hint.packLabel || "thùng").trim() || "thùng";
  const factor = Math.max(
    2,
    Math.round(Number(packFactor) || Number(hint.packFactor) || 10)
  );
  const retail = Math.max(0, Math.round(Number(retailPrice) || 0));
  const cost = Math.max(0, Math.round(Number(baseCost) || 0));
  const packSell =
    Math.round(Number(packPrice) || 0) > 0
      ? Math.round(Number(packPrice) || 0)
      : retail > 0
        ? Math.round(retail * factor)
        : 0;
 ...
 * Đã có packaging: giữ nguyên units (không ghi đè canSell/sellPrice cây↔bao).
 */
export function withReceivePack(product, { unitLabel, packFactor } = {}) {
  ...
  const wantLabel = String(unitLabel || "thùng").trim() || "thùng";
  const wantId = makeUnitId(wantLabel);

  if (norm.enabled && norm.units.length) {
    const byId = norm.units.find((u) => u.id === wantId && u.factor > 1);
    const byLabel = norm.units.find(
      (u) =>
        String(u.label || "").trim().toLowerCase() === wantLabel.toLowerCase() &&
        u.factor > 1
    );
    const byFactor = norm.units.find((u) => Number(u.factor) === factor && u.factor > 1);
    const existing =
      byId || byLabel || byFactor || norm.units.find((u) => u.factor > 1);

    if (existing) {
      if (Number(existing.factor) === factor) {
        return { product, unitId: existing.id };
      }
      const units = norm.units.map((u) =>
        u.id === existing.id ? { ...u, factor } : u
      );
      return {
        product: {
          ...product,
          packaging: { enabled: true, baseUnit: norm.baseUnit },
          units,
        },
        unitId: existing.id,
      };
    ...
    const baseCost = Math.max(0, Number(product?.cost) || 0);
    const newPack = {
      id: wantId,
      label: wantLabel,
      factor,
      sellPrice: 0,
      sellCost: Math.round(baseCost * factor),
      canSell: false,
      canReceive: true,
    };
    return {
  ...
  const pack = buildIngredientPackUnits({
    ...
    canSell: product?.kind === "finished" || Boolean(baseSell?.canSell),
    sellPrice: Number(product?.price) || Number(baseSell?.sellPrice) || 0,
  });
 ...
 * Giá vốn = bình quân gia quyền (WAC) với tồn còn lại:
 *   (tồn × cost cũ + tiền nhập) / (tồn + SL gốc).
 * Tồn ≤ 0 → lấy đơn giá lần nhập này (không có hàng để trộn).
 */
export function deriveReceiveCostUpdate(
  ...
  const purchaseBaseUnitCost = Math.round(amount / baseQty);
  const priorQty = Math.max(0, Number(product?.inStock) || 0);
  const priorCost = Math.max(0, Number(product?.cost) || 0);

  let baseUnitCost = purchaseBaseUnitCost;
  let costMethod = "purchase";
  if (priorQty > 0) {
    const totalValue = priorQty * priorCost + amount;
    const totalQty = priorQty + baseQty;
    baseUnitCost = Math.round(totalValue / totalQty);
    costMethod = "wac";
  }
  ...
  if (normalized.enabled && Array.isArray(normalized.units) && normalized.units.length) {
    ...
    // Đồng bộ sellCost mọi ĐV theo WAC × factor (COGS nhất quán khi bán bao/cây).
    productPatch.units = normalized.units.map((row) => {
      const factor = Math.max(1, Number(row.factor) || 1);
      return {
        ...row,
        sellCost: Math.round(baseUnitCost * factor),
      };
    });
  }
...
export function assertCanSellStock(product, unitId, qty) {
  ...
  // Món công thức trừ NL / thành phẩm kho — không kiểm tồn chính thành phẩm.
  if (productUsesRecipe(product)) return true;
