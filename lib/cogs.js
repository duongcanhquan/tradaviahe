...
export function buildSaleLineFromProduct(
  ...
  const usesRecipe = productUsesRecipe(product);
  const servings = Math.max(1, Number(product.estimatedServings) || 100);
  const recipe = usesRecipe
    ? normalizeRecipe(product.recipe, productsById, {
        estimatedServings: servings,
        convertBatch: true,
      })
    : [];

  let unitCost = resolveSellUnitCost(product, unit.id);
  if (usesRecipe) {
    const fromRecipe = Math.round(
      computeRecipeCost(product.recipe, productsById, null, {
        estimatedServings: servings,
      })
    );
    if (fromRecipe > 0) unitCost = fromRecipe;
    else if (Math.round(Number(product.cost) || 0) > 0) {
      unitCost = Math.round(Number(product.cost) || 0);
    }
  }
  ...
  return {
    ...
    costMode: usesRecipe ? "recipe" : "manual",
    recipe,
    estimatedServings: servings,
  };
}
