...
describe("packaging", () => {
  ...
  it("buildRetailPackPreset thuốc bao + cây bán cả hai", () => {
    const pack = buildRetailPackPreset({
      baseUnit: "bao",
      packLabel: "cây",
      packFactor: 10,
      retailPrice: 25000,
    ...
    const bao = pack.units.find((u) => u.factor === 1);
    const cay = pack.units.find((u) => u.factor === 10);
    assert.equal(bao.sellPrice, 25000);
    assert.equal(bao.canSell, true);
    assert.equal(bao.canReceive, true);
    assert.equal(cay.sellPrice, 230000);
    assert.equal(cay.canSell, true);
    assert.equal(cay.canReceive, true);
  });
  ...
  it("withReceivePack keeps existing cây canSell/sellPrice", () => {
    const existing = {
      name: "Thuốc",
      unit: "bao",
      cost: 18000,
    ...
    const { product, unitId } = withReceivePack(existing, {
      unitLabel: "cây",
      packFactor: 10,
    });
    assert.equal(unitId, "cay");
    const cay = product.units.find((u) => u.id === "cay");
    assert.equal(cay.canSell, true);
    assert.equal(cay.sellPrice, 220000);
    assert.equal(product.units.length, 2);
  });
...
describe("deriveReceiveCostUpdate", () => {
  it("splits carton price into base unit cost when no stock", () => {
    const product = {
      id: "noodle",
      cost: 3500,
      inStock: 0,
    ...
    assert.equal(result.purchaseBaseUnitCost, 4000);
    assert.equal(result.costMethod, "purchase");
  ...
  it("factor 1 uses unit price as cost when no stock", () => {
    const product = { id: "egg", cost: 2000, inStock: 0, unit: "quả", units: [] };
    const unit = {
      id: "base",
      label: "quả",
      factor: 1,
  ...
  it("blends weighted average with remaining stock", () => {
    const product = {
      id: "cig",
      cost: 20000,
      inStock: 10,
    ...
    // (10*20000 + 10*16000) / 20 = 18000
    assert.equal(result.costMethod, "wac");
    assert.equal(result.baseUnitCost, 18000);
    assert.equal(result.purchaseBaseUnitCost, 16000);
    assert.equal(result.priorQty, 10);
    assert.equal(result.priorCost, 20000);
    assert.equal(result.productPatch.cost, 18000);
    assert.equal(
      result.productPatch.units.find((u) => u.id === "bao").sellCost,
      18000
    );
    assert.equal(
      result.productPatch.units.find((u) => u.id === "cay").sellCost,
      180000
    );
  });

  it("WAC when receiving carton over remaining packs", () => {
    const product = {
      id: "noodle",
      cost: 3500,
      inStock: 30,
    ...
    const result = deriveReceiveCostUpdate(product, {
      unit: product.units[1],
      receiveQty: 1,
      unitReceivePrice: 120000,
    });
    // (30*3500 + 120000) / 60 = 3750
    assert.equal(result.costMethod, "wac");
    assert.equal(result.baseQty, 30);
    assert.equal(result.purchaseBaseUnitCost, 4000);
    assert.equal(result.baseUnitCost, 3750);
    assert.equal(
      result.productPatch.units.find((u) => u.id === "thung").sellCost,
      112500
    );
  });

  it("treats negative stock as zero for WAC", () => {
    const product = { id: "x", cost: 5000, inStock: -3, unit: "cái" };
    const unit = { id: "base", label: "cái", factor: 1, canReceive: true };
    const result = deriveReceiveCostUpdate(product, {
      unit,
      receiveQty: 5,
      unitReceivePrice: 4000,
    });
    assert.equal(result.costMethod, "purchase");
    assert.equal(result.baseUnitCost, 4000);
  });
