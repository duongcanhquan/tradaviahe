...
export {
  isShopFundEditableRow as isShopFundEditable,
  shopFundSourceLabel,
  matchesFundSearch,
} from "./fundSearch.js";

/**
 * Sửa khoản nạp/chi quỹ cửa hàng (không đụng phiếu nhập hàng / cặp chuyển).
 */
export async function updateShopFundEntry({
  id,
  amount,
  category,
  note = "",
  ...
  if (!canManageShop(profile?.role)) {
    throw new Error("Không có quyền sửa sổ quỹ");
  }
  if (!id) throw new Error("Thiếu mã giao dịch");
  ...
  if (!snap.exists()) throw new Error("Không tìm thấy giao dịch");
  const row = { id: snap.id, ...snap.data() };
  const { isShopFundEditableRow } = await import("./fundSearch.js");
  if (!isShopFundEditableRow(row)) {
    throw new Error(
      "Khoản này gắn nhập hàng / vốn / chuyển quỹ — sửa ở màn tương ứng hoặc xóa cặp liên quan"
    );
  }
  ...
  const patch = {
    ...
    updatedByName: profile?.name || profile?.username || "",
  };

  if (isShopExpense(row)) {
    const normalized = normalizeExpenseCategory(category || row.category);
    if (normalized === "nhập hàng") {
      throw new Error("Không đổi hạng mục sang nhập hàng tại đây");
    }
    const allowed = MANUAL_EXPENSE_CATEGORIES.some((c) => c.value === normalized);
    if (!allowed) throw new Error("Hạng mục chi không hợp lệ");
  ...
  return { id, amount: value, businessDate };
}
