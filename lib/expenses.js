...
import { canChooseInventoryFundSource, canManageShop } from "./roles";
import {
  inputValueToDateKey,
  dateKeyToInputValue,
...
import { defaultReceiveUnit, findUnit, toBaseQty } from "./packaging.js";
...
export async function receiveInventoryFromShopFund({
  ...
  const baseQty = toBaseQty(receiveQty, unit);
  if (baseQty <= 0) {
    throw new Error("Số lượng nhập phải > 0");
  }

  const amount = Math.round(receiveQty * unitReceivePrice);
  if (amount <= 0) {
  ...
  if (updateCost && Number(unit.factor) === 1) {
    productPayload.cost = unitReceivePrice;
