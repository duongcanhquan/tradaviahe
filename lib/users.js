...
export async function ensureUserProfile(firebaseUser) {
  ...
  // Luồng đăng nhập thường: chỉ cần đúng 1 lần đọc hồ sơ.
  // Không quét Super Admin và không ghi login_index lại ở mọi lần mở app.
  if (snap.exists()) {
    const data = { uid, ...snap.data() };

    // Chỉ cập nhật hồ sơ cũ thiếu username — không ghi ở mỗi lần đăng nhập.
  ...
  // Chỉ tài khoản chưa có hồ sơ mới cần kiểm tra hệ thống đã có SA hay chưa.
  let saCount = null;
  try {
  ...
  const isFirstSuperAdmin = saCount === 0;
  const profile = {
    uid,
    email,
    username: usernameFromEmail,
    name: defaultName,
    role: isFirstSuperAdmin ? ROLES.superadmin : ROLES.employee,
    phone: "",
    note: isFirstSuperAdmin
      ? "Super Admin hệ thống"
      : "Chờ Super Admin cấp quyền",
    active: true,
  ...
  // Hồ sơ mới cần index một lần cho các công cụ quản trị cũ.
  try {
    ...
    // Không chặn đăng nhập nếu index lỗi.
  }
