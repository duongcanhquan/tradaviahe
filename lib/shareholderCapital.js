 ...
 * Nếu dòng đã gắn nạp quỹ cửa hàng — cập nhật luôn giao dịch quỹ đó.
 */
export async function updateCapitalExpense({
  ...
  await updateDoc(capitalRef, payload);

  const fundId = String(row.shopFundTxId || "").trim();
  if (fundId) {
    ...
    if (fundSnap.exists()) {
      ...
      if (payload.dateKey) fundPatch.dateKey = payload.dateKey;
      await updateDoc(fundRef, fundPatch);
    }
  }
}
 ...
 * Nếu đã nạp quỹ cửa hàng — xóa luôn giao dịch nạp đó (tiền về đúng sổ).
...
export async function deleteCapitalExpense({ entryId, role }) {
  ...
  return { id: entryId, removedFundTxId: fundId || null };
}
