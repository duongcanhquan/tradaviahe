...
/**
 * Sửa vốn ban đầu / thêm vốn / ghi·sửa·xóa chi tiêu vốn — chỉ Super Admin.
...
/**
 * Khi nhập hàng: Admin (Chủ ĐT) / Super Admin được chọn trừ quỹ cửa hàng
 * hoặc quỹ đầu tư. Quản lý chỉ trừ quỹ cửa hàng.
 */
export function canChooseInventoryFundSource(role) {
...
/** Xóa dòng chi/nạp quỹ cửa hàng — chỉ Super Admin (hoàn tiền về quỹ). */
export function canDeleteShopFundEntry(role) {
  return role === "superadmin";
}
...
/** Kiểm kho (đếm thực tế, ghi lệch) — Quản lý + Chủ ĐT + Super Admin. */
export function canStocktake(role) {
  return roleAtLeast(role, "manager");
}
