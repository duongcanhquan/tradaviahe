...
describe("recipe usage → cost + stock", () => {
  ...
  it("legacy batch lines convert to per-serving and deduct", () => {
    ...
    assert.equal(deltas.tea, -3);
    assert.equal(deltas.sugar, -15);
  });

  it("deducts thành phẩm nhập used in recipe even without costMode", () => {
    const deltas = stockDeltasForSaleItems(
      [
        {
          productId: "pho",
          qty: 2,
          recipe: [
            { productId: "noodle-pack", qty: 1, baseQty: 1 },
            { productId: "egg", qty: 1, baseQty: 1 },
          ],
        },
      ],
      -1
    );
    assert.equal(deltas["noodle-pack"], -2);
    assert.equal(deltas.egg, -2);
    assert.equal(deltas.pho, undefined);
  });
