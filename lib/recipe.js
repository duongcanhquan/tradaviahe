...
/** Món bán dùng CT (kể cả bản ghi cũ thiếu costMode). Hàng nhập bán nguyên = manual. */
export function productUsesRecipe(product) {
  if (!product || product.kind === "ingredient") return false;
  const lines = Array.isArray(product.recipe) ? product.recipe : [];
  if (lines.some((line) => line && (line.virtual || line.productId))) {
    return true;
  }
  if (product.costMode === "manual") return false;
  return product.costMode === "recipe" || !product.costMode;
}

/**
 * Hàng được nhập kho / trừ quỹ: nguyên liệu + thành phẩm nhập.
 * Món POS công thức (bát mì…) không nhập — tạo từ NL khi bán.
 */
export function isInventoryReceivable(product) {
...
function emptyRecipeStockLine() {
  return { productId: "", qty: "1", unitId: "", virtual: false };
}

/** Dòng CT trống để form sửa món cũ thêm ngay được. */
export function ensureRecipeDraftLines(recipe) {
  const rows = Array.isArray(recipe) ? recipe.filter(Boolean) : [];
  return rows.length ? rows : [emptyRecipeStockLine()];
}
